...
// ─── Analytics ────────────────────────────────────────────────────────────────
// Fire-and-forget event tracker. Writes to analytics_events table in Supabase.
// Silently no-ops in demo mode or if Supabase is unavailable.
function track(event, properties) {
    if (isDemoMode || !supabaseClient) return;
    try {
        var sid = sessionStorage.getItem('_ip_sid');
        if (!sid) { sid = Math.random().toString(36).slice(2) + Date.now().toString(36); sessionStorage.setItem('_ip_sid', sid); }
        supabaseClient.from('analytics_events').insert({
            user_id: (AppState && AppState.user) ? AppState.user.id : null,
            session_id: sid,
            event: event,
            properties: properties || {}
        }); // fire-and-forget — no await, never blocks the UI
...
function getEffectiveWeight(e, t) {
    ...
    // bodyweight exercises log actual weight directly (bodyweight or bodyweight+vest)
    return t || 0
}
...
async function handleAuthSignUp(e, t, a, s, referralCode) {
    if (isDemoMode || !supabaseClient) return {
        error: {
...
async function initApp() {
    // Capture referral code from URL (?ref=USERNAME) before the hash router takes over
    try {
        const _refParam = new URLSearchParams(window.location.search).get('ref');
        if (_refParam) { try { localStorage.setItem('_ip_ref', _refParam); } catch(e){} }
    } catch(e){}
...
function renderSignup(e) {
    ...
    a.addEventListener("input", () => {
        ...
        if (isDemoMode) AppState.isAuthenticated = !0, AppState.onboardingComplete = !1, AppState.user = {
            ...
            const _refCode = (() => { try { return localStorage.getItem('_ip_ref'); } catch(e){ return null; } })();
            const {
                data: r,
                error: l
            } = await handleAuthSignUp(s, n, t, a, _refCode);
            if (l) return i.textContent = l.message || "Signup failed", i.style.display = "block", o.disabled = !1, void(o.textContent = "Create Account");
            track('signup_completed', { method: 'email', via_referral: !!_refCode });
            // Process referral bonus: find referrer by username and award tokens to both
            if (_refCode && AppState.user && !isDemoMode && supabaseClient) {
                try {
                    const { data: referrer } = await supabaseClient.from('profiles').select('id').eq('username', _refCode).maybeSingle();
                    if (referrer && referrer.id !== AppState.user.id) {
                        await supabaseClient.rpc('increment_tokens', { uid: AppState.user.id, delta: 25 });
                        await supabaseClient.rpc('increment_tokens', { uid: referrer.id, delta: 25 });
                        await supabaseClient.from('profiles').update({ referred_by: referrer.id }).eq('id', AppState.user.id);
                        showToast('You and your friend both earned 25 tokens!', 'success', 4000);
                    }
                    try { localStorage.removeItem('_ip_ref'); } catch(e){}
...
function renderDashboard(e) {
    ...
    var _hasW=AppState.workouts.length>0,_hasF=(AppState.friends||[]).length>0,_hasP=(AppState.pacts||[]).length>0,_hasC=(AppState.coachingLogs||[]).length>0,_onbDone=[_hasW,_hasF,_hasP,_hasC].filter(Boolean).length;
    var _onbSteps=[{label:"Log your first workout",done:_hasW,href:"#/workout/new",reward:"+10 tokens"},{label:"Add a friend",done:_hasF,href:"#/social",reward:"+5 tokens"},{label:"Join or create a Pact",done:_hasP,href:"#/social",reward:"+10 tokens"},{label:"Try the AI Coach",done:_hasC,href:"#/coach",reward:"+5 tokens"}];
    var _onboarding=n<5&&_onbDone<4?'<div class="card" style="margin-bottom:24px;border:1px solid var(--accent);">'+'<div class="card-header" style="padding-bottom:12px;">'+'<h3 class="card-title">Getting Started</h3>'+'<span style="font-size:12px;color:var(--text-secondary);">'+_onbDone+'/4 complete</span>'+'</div><div style="display:flex;flex-direction:column;gap:6px;">'+_onbSteps.map(function(s){return '<a href="'+s.href+'" style="display:flex;align-items:center;gap:12px;padding:10px 12px;border-radius:var(--radius-md);background:'+(s.done?'transparent':'var(--bg-secondary)')+';text-decoration:none;opacity:'+(s.done?'0.5':'1')+'"><span style="font-size:16px;">'+(s.done?'Done ':'○')+'</span><span style="flex:1;font-size:14px;color:var(--text-primary);">'+s.label+'</span>'+'<span style="font-size:11px;color:var(--accent);font-weight:600;">'+(s.done?'Complete':s.reward)+'</span></a>';}).join('')+'</div></div>':'';
...
function renderNewWorkout(e) {
    ...
    render(), window.removeExerciseFromWorkout = e => {
        ...
        if (!isDemoMode && supabaseClient && AppState.user) {
            ...
            if (o) await sbLoadWorkouts(), await sbLoadPersonalRecords(), AppState.activeWorkout = null, AppState.activeWorkoutStartTime = null, clearActiveWorkoutStorage(), showToast("Workout completed! Great work. 💪"), (AppState.workouts.length===1&&track("first_workout_logged",{})), showWorkoutCompletionModal(n), setTimeout(() => {
...
function showWorkoutCompletionModal(e) {
    ...
    a.className = "modal-overlay", a.innerHTML = `\n    <div class="modal" style="max-width:480px;">\n      <div class="modal-header">\n        <span class="modal-title">🏆 Workout Complete!</span>\n        <button class="modal-close" id="close-completion">✕</button>\n      </div>\n      <div class="modal-body">\n        <div style="text-align:center;margin-bottom:20px;">\n          <div style="font-size:48px;margin-bottom:8px;">💪</div>\n          <div style="font-family:var(--font-display);font-weight:700;font-size:18px;">${e.name}</div>\n          <div style="color:var(--text-secondary);font-size:13px;">${Math.max(1,Math.round((e.duration_seconds||0)/60))} min · ${(e.exercises||[]).length} exercises</div>\n        </div>\n        <div style="margin-bottom:12px;font-size:13px;font-weight:600;color:var(--text-secondary);text-transform:uppercase;letter-spacing:0.5px;">Coach's Tips for This Session</div>\n        <div class="coaching-tips-list">\n          ${t.map(e=>`\n            <div class="coaching-tip-card">\n              <div class="coaching-tip-icon">${getCoachingTypeIcon(e.coaching_type)}</div>\n              <div class="coaching-tip-body">\n                <div class="coaching-tip-label">${getCoachingTypeLabel(e.coaching_type)}</div>\n                <div class="coaching-tip-message">${e.message}</div>\n              </div>\n            </div>\n          `).join("")}\n        </div>\n        <div style="display:flex;gap:8px;margin-top:16px;"><button class="btn btn-secondary" id="share-workout-btn" style="flex:1;">Share</button><button class="btn btn-primary" id="close-completion-btn" style="flex:2;">Got It!</button></div>\n      </div>\n    </div>\n  `; document.body.appendChild(a); var _wVol=getTotalVolume(e),_wDur=Math.max(1,Math.round((e.duration_seconds||0)/60)),_wTxt="Crushed "+e.name+" - "+(e.exercises||[]).length+" exercises, "+formatWeight(_wVol)+" volume in "+_wDur+"min #IronPact"; a.querySelector("#share-workout-btn").addEventListener("click",function(){if(navigator.share){navigator.share({title:"IronPact Workout",text:_wTxt,url:"https://ironpact-xi.vercel.app"}).catch(function(){});}else if(navigator.clipboard){navigator.clipboard.writeText(_wTxt+" https://ironpact-xi.vercel.app").then(function(){showToast("Copied to clipboard!","success");}).catch(function(){});} track("workout_shared",{workout_id:e.id});}); a.querySelector("#close-completion").addEventListener("click",function(){a.remove();}); a.querySelector("#close-completion-btn").addEventListener("click",function(){a.remove();}); a.addEventListener("click",function(ev){if(ev.target===a)a.remove();});
}
...
function renderStats(e) {
    ...
    e.innerHTML = `\n    <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px;margin-bottom:24px;">\n      <div>\n        <div class="page-title">Analytics</div>\n        <div class="page-subtitle">Track your progress over time</div>\n      </div>\n      ${renderUnitToggle()}\n    </div>\n\n    <div class="stats-grid" style="margin-bottom:24px;">\n      <div class="stat-card">\n        <div class="stat-label">Total Workouts</div>\n        <div class="stat-value">${t}</div>\n        <div class="stat-change" style="font-size:11px;color:var(--text-secondary)">This week: ${_workoutsThisWeek}</div>\n      </div>\n      <div class="stat-card">\n        <div class="stat-label">Login Streak</div>\n        <div class="stat-value"><span class="streak-fire">🔥</span> ${a}d</div>\n        <div class="stat-change" style="color:var(--text-secondary);font-size:11px;">Best: ${s}d · Workout: ${wStreak}d</div>\n      </div>\n      <div class="stat-card">\n        <div class="stat-label">This Month</div>\n        <div class="stat-value">${d}</div>\n        <div class="stat-change positive">${_bestMonth ? 'Best: '+_bestMonth[1]+' ('+_bestMonth[0]+')' : 'workouts'}</div>\n      </div>\n      <div class="stat-card">\n        <div class="stat-label">Total Volume</div>\n        <div class="stat-value">${formatVolumeK(n)}<span style="font-size:14px;color:var(--text-secondary);font-weight:400;"> ${i}</span></div>\n      </div>\n      <div class="stat-card">\n        <div class="stat-label">Avg Duration</div>\n        <div class="stat-value">${_avgDuration}<span style="font-size:14px;color:var(--text-secondary);font-weight:400;"> min</span></div>\n      </div>\n      <div class="stat-card">\n        <div class="stat-label">PRs</div>\n        <div class="stat-value">🏆 ${_totalPRs}</div>\n        <div class="stat-change positive">exercises</div>\n      </div>\n    </div>\n\n    <div class="analytics-grid">\n      <div class="card">\n        <div class="card-header">\n          <div class="card-title">Volume Over Time</div>\n          <span style="font-size:11px;color:var(--text-tertiary);">Last 12 weeks</span>\n        </div>\n        <div class="chart-container">\n          <canvas id="volume-chart"></canvas>\n        </div>\n      </div>\n      <div class="card">\n        <div class="card-header">\n          <div class="card-title">Muscle Distribution</div>\n          <span style="font-size:11px;color:var(--text-tertiary);">All time</span>\n        </div>\n        <div class="chart-container">\n          <canvas id="muscle-chart"></canvas>\n        </div>\n      </div>\n    </div>\n\n    <div class="card" style="margin-top:24px;">\n      <div class="card-header">\n        <div class="card-title">\n    <div class="analytics-grid" style="margin-top:24px;">\n      <div class="card">\n        <div class="card-header">\n          <div class="card-title">Workouts Per Week</div>\n          <span style="font-size:11px;color:var(--text-tertiary);">Last 12 weeks</span>\n        </div>\n        <div class="chart-container"><canvas id="frequency-chart"></canvas></div>\n      </div>\n      <div class="card">\n        <div class="card-header">\n          <div class="card-title">Avg Duration / Week</div>\n          <span style="font-size:11px;color:var(--text-tertiary);">Minutes per week</span>\n        </div>\n        <div class="chart-container"><canvas id="duration-chart"></canvas></div>\n      </div>\n    </div>\n\n    <div class="card" style="margin-top:24px;">\n      <div class="card-header">\n        <div class="card-title">Workout Heatmap</div>\n        <span style="font-size:11px;color:var(--text-tertiary);">Last 90 days</span>\n      </div>\n      <div class="heatmap-container" id="workout-heatmap">\n        ${renderHeatmap(c)}\n      </div>\n    </div>\n\n    ${Object.keys(AppState.personalRecords).length>0?`\n      <div class="card" style="margin-top:24px;">\n        <div class="card-header">\n          <div class="card-title">Personal Records</div>\n          <span style="font-size:11px;color:var(--text-tertiary);">${Object.keys(AppState.personalRecords).length} exercises</span>\n        </div>\n        <div class="exercise-grid">\n          ${Object.entries(AppState.personalRecords).map(([e,t])=>{const a=AppState.exercises.find(t=>t.id===e),s=a?a.name:t.exercise_name||"Unknown Exercise";return 0===t.max_weight?"":`\n              <div class="exercise-card" style="position:relative;">\n                <div class="exercise-card-icon" style="background:var(--accent-glow);border-radius:var(--radius-md);color:var(--accent);">🏆</div>\n                <div class="exercise-card-info">\n                  <div class="exercise-card-name">${s}</div>\n                  <div class="exercise-card-detail">\n                    <span class="text-accent" style="font-weight:700;">${formatWeight(t.max_weight)}</span>\n                    <span style="color:var(--text-tertiary);margin:0 4px;">·</span>\n                    <span>Reps @ max: ${t.max_weight_reps || t.max_reps || "\u2014"}</span>\n                  </div>\n                </div>\n                <button class="btn btn-ghost btn-sm delete-pr-btn" data-exid="${e}" style="position:absolute;top:6px;right:4px;color:var(--text-tertiary);padding:4px;font-size:16px;line-height:1;" title="Remove PR">×</button>\n              </div>\n            `}).join("")}\n        </div>\n      </div>\n    `:'\n      <div class="card" style="margin-top:24px;">\n        <div class="empty-state" style="padding:32px;">\n          <div class="empty-state-icon">📊</div>\n          <div class="empty-state-title">No stats yet</div>\n          <div class="empty-state-text">Log some workouts to see your analytics here.</div>\n          <a href="#/workout/new" class="btn btn-primary" style="margin-top:16px;">Start a Workout</a>\n        </div>\n      </div>\n    '}\n  `, setTimeout(() => {
        renderVolumeChart(o), renderMuscleChart(r), renderFrequencyChart(_weeklyCount), renderDurationChart(_weeklyCount, o);
...
function renderProfile(e) {
    ...
    // Referral invite card handlers
    const copyBtn = document.getElementById("copy-referral-btn");
    if (copyBtn) {
        copyBtn.addEventListener("click", () => {
            const inp = document.getElementById("referral-url-input");
            if (inp) { inp.select(); try { navigator.clipboard.writeText(inp.value).then(() => showToast("Invite link copied!", "success")).catch(() => { document.execCommand("copy"); showToast("Invite link copied!", "success"); }); } catch(ex){ document.execCommand("copy"); showToast("Invite link copied!", "success"); } }
        });
    }
    ...
    if (shareBtn) {
        shareBtn.addEventListener("click", async () => {
            ...
            try { await navigator.share({ title: "Join me on IronPact", text: "Track workouts, earn tokens, and compete in pacts. Use my link for 25 bonus tokens!", url }); } catch(ex){}
        });
    }
}
...
function renderShop(e) {
    ...
    ! function render() {
        ...
        e.innerHTML = `\n      <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px;margin-bottom:24px;">\n        <div>\n          <div class="page-title">Cosmetics Shop 🛒</div>\n          <div class="page-subtitle">Spend tokens to unlock cosmetics</div>\n        </div>\n        <div class="token-balance-card">\n          <span class="token-balance-icon">⬡</span>\n          <span class="token-balance-amount token-balance-display">${AppState.tokenBalance}</span>\n          <span class="token-balance-label">tokens</span>\n          ${t?'<span class="badge-beta">BETA</span>':""}\n        </div>\n      </div>\n\n      ${t?'<div class="beta-notice">🎖️ <strong>Beta Access:</strong> All cosmetics are FREE for you! Costs shown are for regular users.</div>':""}\n\n      <div class="filter-row" style="margin-bottom:20px;">\n        ${["all","owned","badge","theme","title","frame"].map(e=>`<button class="filter-chip ${a===e?"active":""}" data-cat="${e}">${s[e]}</button>`).join("")}\n      </div>\n\n      <div class="shop-grid">\n        ${0===n.length&&"owned"===a?'\n          <div style="grid-column:1/-1;padding:40px;text-align:center;color:var(--text-secondary);">\n            <div style="font-size:32px;margin-bottom:12px;">🎒</div>\n            <div style="font-weight:600;margin-bottom:6px;">No items in your collection yet</div>\n            <div style="font-size:13px;">Purchase items from the shop to build your collection.</div>\n          </div>\n        ':n.map(e=>{const a=i[e.rarity]||i.common,s=isOwned(e.id),n=isEquipped(e.id),o=e.cost,r=t||AppState.tokenBalance>=o||0===o;return`\n            <div class="shop-item" style="border-color:${a.border};background:${a.bg};box-shadow:${a.shadow};">\n              <div class="shop-item-rarity" style="color:${a.text};">${a.label}</div>\n              <div class="shop-item-icon">${e.icon||"🎁"}</div>\n              <div class="shop-item-name">${e.name}</div>\n              <div class="shop-item-desc">${e.description}</div>\n              <div class="shop-item-cost">\n                ${0===o?'<span style="color:var(--success);font-weight:700;">FREE</span>':t?`<span style="color:var(--success);font-weight:700;">FREE</span> <span style="text-decoration:line-through;color:var(--text-tertiary);font-size:11px;">${o} ⬡</span>`:`<span class="token-icon-sm">⬡</span> <span style="font-weight:700;">${o}</span>`}\n              </div>\n              <div class="shop-item-actions">\n                ${n?`<button class="btn btn-secondary btn-sm w-full shop-unequip-btn" data-cid="${e.id}">Unequip</button>`:s?`<button class="btn btn-primary btn-sm w-full shop-equip-btn" data-cid="${e.id}">Equip</button>`:`<button class="btn btn-primary btn-sm w-full shop-buy-btn ${r?"":"disabled-btn"}" data-cid="${e.id}" ${r?"":"disabled"}>${r?"Purchase":"Need More ⬡"}</button>`}\n              </div>\n            </div>\n          `}).join("")}\n      </div>\n\n      <div style="margin-top:32px;">\n        <div style="font-family:var(--font-display);font-size:18px;font-weight:700;margin-bottom:6px;">⬡ Get Tokens</div>\n        <div style="font-size:13px;color:var(--text-secondary);margin-bottom:20px;">Top up instantly — buy once, keep forever. No subscription required.</div>\n        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:12px;">\n          ${[{id:"starter",tokens:150,price:"$0.99",label:"Starter"},{id:"power",tokens:400,price:"$2.49",label:"Power",popular:true},{id:"elite",tokens:900,price:"$4.99",label:"Elite"},{id:"champion",tokens:2000,price:"$9.99",label:"Champion"},{id:"legendary",tokens:5500,price:"$24.99",label:"Legendary",best:true}].map(b=>`\n            <div style="background:rgba(255,255,255,0.04);border:1px solid ${b.popular?"var(--accent)":b.best?"#FFD700":"var(--border)"};border-radius:var(--radius-md);padding:16px 12px;text-align:center;position:relative;">\n              ${b.popular?'<div style="position:absolute;top:-10px;left:50%;transform:translateX(-50%);background:var(--accent);color:#fff;font-size:10px;font-weight:700;padding:2px 10px;border-radius:20px;white-space:nowrap;">POPULAR</div>':""}\n              ${b.best?'<div style="position:absolute;top:-10px;left:50%;transform:translateX(-50%);background:#FFD700;color:#000;font-size:10px;font-weight:700;padding:2px 10px;border-radius:20px;white-space:nowrap;">BEST VALUE</div>':""}\n              <div style="font-weight:700;font-size:14px;margin-bottom:6px;">${b.label}</div>\n              <div style="font-size:24px;font-weight:800;color:var(--accent);">${b.tokens.toLocaleString()}</div>\n              <div style="font-size:11px;color:var(--text-secondary);margin-bottom:12px;">tokens</div>\n              <button class="btn btn-primary btn-sm w-full token-bundle-btn" data-bundle="${b.id}" data-price="${b.price}" style="font-size:13px;padding:8px 0;font-weight:700;">${b.price}</button>\n            </div>\n          `).join("")}\n        </div>\n      </div>\n\n      <div style="margin-top:24px;">\n        <button class="btn btn-accent w-full" id="mystery-box-btn" style="background:linear-gradient(135deg,#a855f7,#ec4899);border:none;color:#fff;padding:14px;font-size:15px;font-weight:700;border-radius:var(--radius-md);cursor:pointer;display:flex;align-items:center;justify-content:center;gap:10px;">\n          <span style="font-size:20px;">🎁</span> Mystery Box — 30 ⬡\n        </button>\n        <div style="margin-top:10px;font-size:12px;color:var(--text-secondary);text-align:center;line-height:1.7;">\n          Cosmetics guaranteed — but every box has a chance to drop something rarer.<br>\n          <span style="color:#2196F3;">◆ Token Cache (75⬡)</span> &nbsp;·&nbsp; <span style="color:#CE93D8;">◆ Token Vault (200⬡)</span> &nbsp;·&nbsp; <span style="color:#FFD700;">◆ Token Jackpot (500⬡)</span> &nbsp;·&nbsp; <span style="color:#FF6B6B;font-weight:700;">★ Free 1-Week Premium</span>\n        </div>\n      </div>\n    `, e.querySelectorAll(".filter-chip").forEach(e => {
            e.addEventListener("click", () => {
        ...
        o && o.addEventListener("click", async () => {
            ...
            // ── Special prizes (rare / epic / legendary token drops + ultra-rare membership) ──
            let sp = null;
            if (a >= 99.8) sp = {type:"membership",tier:"ULTRA RARE",icon:"👑",name:"1 Week Free Premium!",desc:"Full premium access activated for 1 week — on the house.",color:"#FF6B6B"};
            else if (a >= 99.0) sp = {type:"tokens",amount:500,tier:"LEGENDARY",icon:"💰",name:"Token Jackpot",desc:"+500 tokens added to your balance.",color:"#FFD700"};
            else if (a >= 97.0) sp = {type:"tokens",amount:200,tier:"EPIC",icon:"⬡",name:"Token Vault",desc:"+200 tokens added to your balance.",color:"#CE93D8"};
            else if (a >= 94.0) sp = {type:"tokens",amount:75,tier:"RARE",icon:"⬡",name:"Token Cache",desc:"+75 tokens added to your balance.",color:"#2196F3"};
            if (sp) {
                t || await sbSpendTokens(30, "mystery_box");
                if (sp.type==="tokens") await sbEarnTokens(sp.amount, "mystery_box_prize");
                else if (sp.type==="membership" && !t && AppState.user) {
                    await supabaseClient.from("profiles").update({is_premium:true,subscription_plan:"weekly"}).eq("id",AppState.user.id);
                    Object.assign(AppState.profile,{is_premium:true,subscription_plan:"weekly"});
                }
                e.innerHTML = `\n          <div style="font-size:72px;animation:pop 0.5s ease;">${sp.icon}</div>\n          <div style="font-size:22px;font-weight:800;color:${sp.color};text-transform:uppercase;letter-spacing:2px;">${sp.tier}</div>\n          <div style="font-size:20px;font-weight:700;color:#fff;margin-top:4px;">${sp.name}</div>\n          <div style="font-size:13px;color:#aaa;">${sp.desc}</div>\n          <button id="mystery-close" style="margin-top:20px;padding:12px 32px;background:var(--accent);color:#fff;border:none;border-radius:8px;font-size:15px;font-weight:700;cursor:pointer;">Awesome!</button>\n          <style>@keyframes pop{0%{transform:scale(0)}60%{transform:scale(1.2)}100%{transform:scale(1)}}</style>\n        `;
                e.querySelector("#mystery-close").addEventListener("click", () => { e.remove(), render() });
                return;
            }
            // ── Regular cosmetic prize ──
            let s;
            s = a < 45 ? "common" : a < 72 ? "rare" : a < 86 ? "epic" : "legendary";
    ...
    window.showExercisePicker = function() {
      ...
      setTimeout(function() {
        ...
        modal.addEventListener('click', function(e) {
          ...
          // After original handler adds exercise, fill weight_kg with bodyweight if still 0
          setTimeout(function() {
...
(function(){
  ...
  handleRoute = function handleRoute() {
    ...
    if (hash === '/workout/new' && AppState.activeWorkout) {
      ...
      if (hasBWExercise && !AppState.profile.body_weight && !AppState.profile.weight_kg) {
        var isImperial = AppState.unitPref === 'imperial' || AppState.profile.unit_pref === 'imperial';
        var u = isImperial ? 'lbs' : 'kg';
      ...
      // Fill BW exercise sets that have weight_kg=0 with stored bodyweight
      if (hasBWExercise) {
        var bwKg = getBodyweightKg();
        (AppState.activeWorkout.exercises || []).forEach(function(ex) {
          var exObj = (AppState.exercises || []).find(function(e) { return e.id === ex.exercise_id; });
...
(function(){
  ...
  // NOTE: personal_records only allows record_type IN ('max_weight', 'max_volume').
  // We store the e1rm value under 'max_weight' so comparisons are e1rm-based
...
// ═══════════════════════════════════════════════════════════════
// PATCH: Editable active workout timer
// ═══════════════════════════════════════════════════════════════
window.editWorkoutTimer = function() {
  if (!AppState.activeWorkout) return;
  var elapsed = AppState.activeWorkoutStartTime
    ? (AppState.activeWorkoutPaused
        ? AppState.activeWorkoutElapsedBeforePause
        : Math.floor(AppState.activeWorkoutElapsedBeforePause + (Date.now() - AppState.activeWorkoutStartTime) / 1000))
    : AppState.activeWorkoutElapsedBeforePause || 0;
  var curMins = Math.round(elapsed / 60);
  var newVal = window.prompt('Correct workout duration (minutes):', String(curMins));
  if (newVal === null) return;
  var mins = parseInt(newVal, 10);
  if (isNaN(mins) || mins < 0) { showToast('Invalid duration', 'error'); return; }
  var newSecs = mins * 60;
  AppState.activeWorkoutElapsedBeforePause = newSecs;
  if (!AppState.activeWorkoutPaused) {
    AppState.activeWorkoutStartTime = Date.now();
  }
  if (typeof saveActiveWorkoutToStorage === 'function') saveActiveWorkoutToStorage();
